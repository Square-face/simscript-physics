...
impl Panel {
    ...
    pub fn to_moment(&self, velocity: &Velocity, rotation: Quat) -> Moment {
        let rotated = self.rotated(&rotation);
        let vel = rotated.tip_velocity(velocity);

        let force = rotated.to_force(&vel);

        Moment::from_force_and_offset(force, rotation.mul_vec3(self.offset))
    }
}
...
mod to_moment {
    ...
    fn stationary() {
        ...
        assert_ulps_eq!(px.to_moment(&Velocity::ZERO, Quat::IDENTITY), Moment::ZERO);
    }
    ...
    fn linear_movement() {
        ...
        assert_ulps_eq!(px.to_moment(&vy, q), Moment::ZERO);
        assert_ulps_eq!(px.to_moment(&vz, q), Moment::ZERO);

        assert_ulps_eq!(py.to_moment(&vx, q), Moment::ZERO);
        assert_ulps_eq!(
            py.to_moment(&vy, q),
            Moment::from_force(Force::new(Vec3::NEG_Y * EXP))
        );
        assert_ulps_eq!(py.to_moment(&vz, q), Moment::ZERO);

        assert_ulps_eq!(pz.to_moment(&vx, q), Moment::ZERO);
        assert_ulps_eq!(pz.to_moment(&vy, q), Moment::ZERO);
        assert_ulps_eq!(
            pz.to_moment(&vz, q),
    ...
    fn angular_movement() {
        ...
        assert_ulps_eq!(pxy.to_moment(&vx, q).magnitude(), 0.);
        assert_ulps_eq!(pxy.to_moment(&vy, q).magnitude(), 0.);
        assert_ulps_eq!(
            pxy.to_moment(&vz, q),
        ...
        assert_ulps_eq!(pyz.to_moment(&vy, q).magnitude(), 0.);
        assert_ulps_eq!(pyz.to_moment(&vz, q).magnitude(), 0.);

        assert_ulps_eq!(pzx.to_moment(&vx, q).magnitude(), 0.);
        assert_ulps_eq!(
            pzx.to_moment(&vy, q),
            Moment::from_force_and_offset(Force(Vec3::NEG_X * EXP), Vec3::Z)
        );
        assert_ulps_eq!(pzx.to_moment(&vz, q).magnitude(), 0.);
    }
}
